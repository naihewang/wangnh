 ...
 * invokes the {@link
 * RejectedExecutionHandler#rejectedExecution(Runnable, mine.ThreadPoolExecutor)}
...
public class ThreadPoolExecutor extends AbstractExecutorService {
    ...
    //返回值总结：
    //true 表示创建worker成功，且线程启动成功

    //false 表示创建失败。
    //1.线程池状态rs > SHUTDOWN (STOP/TIDYING/TERMINATION)
    //2.rs == SHUTDOWN 提交的firstTask不为空；rs == SHUTDOWN 提交的firstTask为空，但是任务队列不为空
    ...
    private Runnable getTask() {
        ...
        for (;;) {
            ...
            try {
                //获取任务的逻辑，poll支持超时可以回收；take不会超时，会阻塞不会被回收
    ...
    final void runWorker(Worker w) {
        ...
        try {
            ...
            //什么情况下，会来到这里？
            //getTask()方法返回null时，说明当前线程应该执行退出逻辑了。正常退出
    ...
    public static class CallerRunsPolicy implements RejectedExecutionHandler {
         ...
         */
        public void rejectedExecution(Runnable r, mine.ThreadPoolExecutor e) {
            if (!e.isShutdown()) {
                r.run();
            }
        }
